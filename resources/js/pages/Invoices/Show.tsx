...
export default function InvoiceShow({ invoice }: Props) {
  ...
  /** ⚠️ Transitions strictement alignées sur InvoiceController::$allowedTransitions */
  ...
  /* ------------------------- Actions ------------------------------- */
  const exportPdf = () =>
    window.open(route('invoices.export', invoice.id), '_blank', 'noopener')
...
const Header = ({
  ...
  >
    ...
    <div className="flex flex-col gap-2 w-full sm:w-auto">
      ...
      <a
  href={route('invoices.export-pdf', invoice.id)}
  target="_blank"
  rel="noopener"
...
>
  <Button variant="secondary" className="w-full sm:w-auto">
    ...
    Exporter&nbsp;PDF
      ...
      {(['sent', 'issued', 'partially_paid'].includes(invoice.status) || !!invoice.is_overdue) && (
        <Button
          variant="secondary" className="w-full sm:w-auto"
          onClick={markAsPaid}
        >
      ...
      {/* Bouton dédié — Rembourser (ouvre la popup de commentaire + transition paid → refunded) */}
      {invoice.status === 'paid' && (
        <Button
          variant="secondary"
          className="w-full sm:w-auto"
      ...
      {transitions[invoice.status]?.length ? (
