...
export default function TaxRatesIndex({ taxRates, flash }: Props) {
  ...
  const filteredTaxRates = useMemo(() => {
    return taxRatesArray.filter((taxRate) =>
      activeFilters.every((f) => {
        if (f.field === 'name')
          return taxRate.name.toLowerCase().includes(f.value.toLowerCase());
        if (f.field === 'status')
  ...
  const deleteSelectedTaxRates = () => {
    ...
    Promise.all(
      selectedTaxRateIds.map((id) =>
        router.delete(route('taxrates.destroy', { id }), { preserveScroll: true }),
      ),
    )
      .then(() => setSelectedTaxRateIds([]))
      .catch(() =>
        alert('Une erreur est survenue lors de la suppression de certains taux de TVA.'),
      );
  };
  ...
  const restoreSelectedTaxRates = () => {
    ...
    Promise.all(
      ...
      .then(() => setSelectedTaxRateIds([]))
      .catch(() =>
        alert('Une erreur est survenue lors de la restauration de certains taux de TVA.'),
      );
  };
  ...
  const toggleSelectAll = () => {
    ...
    setSelectedTaxRateIds((p) =>
      p.length === paginatedTaxRates.length ? [] : paginatedTaxRates.map((r) => r.id),
    );
  };

  const allSelected =
    selectedTaxRateIds.length && paginatedTaxRates.every((r) => selectedTaxRateIds.includes(r.id));

  const anySelectedInactive = selectedTaxRateIds.some(
    (id) => taxRatesArray.find((r) => r.id === id)?.deleted_at,
  );
  const anySelectedActive = selectedTaxRateIds.some(
    (id) => !taxRatesArray.find((r) => r.id === id)?.deleted_at,
  );
            ...
            <div className="flex items-center gap-3 mb-6">
              <div>
                ...
                <p className="text-sm text-slate-600 dark:text-slate-400">
                  Administration et gestion des taux de taxation
                </p>
              </div>
            </div>
                  ...
                  {showFilterPanel && (
                    <div className="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-4 w-full lg:max-w-xl">
                      <h3 className="text-sm font-medium text-slate-700 dark:text-slate-200 mb-3 flex items-center gap-2">
                        <SlidersHorizontal className="w-4 h-4" /> Filtrer les taux de TVA
                      </h3>
                ...
                <div className="flex items-center gap-3 ml-auto">
                  ...
                  <Link href="/tax-rates/create">
                    <Button className="bg-gradient-to-r from-red-600 to-red-500 text-white hover:from-red-500 hover:to-red-600 shadow-md">
                      <Plus className="w-4 h-4 mr-1" /> Ajouter un taux
                    </Button>
                  </Link>
                </div>
              </div>
                  ...
                  {paginatedTaxRates.length ? (
                    paginatedTaxRates.map((taxRate) => (
                      <tr
                        key={taxRate.id}
                        className={`${
                          taxRate.deleted_at ? 'bg-red-50 dark:bg-red-900/10' : ''
