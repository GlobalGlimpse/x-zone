...
export default function CategoriesIndex({ categories, flash }: Props) {
  ...
  const removeFilter = (idx: number) =>
    setActiveFilters((p) => p.filter((_, i) => i !== idx));
  ...
  const toggleSelectAll = () => {
    ...
    setSelectedIds((p) =>
      p.length === paginated.length ? [] : paginated.map((c) => c.id),
    );
  };
            ...
            <div className="flex items-center gap-3 mb-6">
              <div>
                ...
                <p className="text-sm text-slate-600 dark:text-slate-400">
                  Administration et gestion des catégories système
                </p>
              </div>
            </div>
                  ...
                  {paginated.length === 0 ? (
                    ...
                    paginated.map((cat) => (
                      <tr
                        key={cat.id}
                        className={`${
                          cat.deleted_at ? 'bg-red-50 dark:bg-red-900/10' : ''
                  ...
                  <span className="text-slate-600 dark:text-slate-300">
                    {rowsPerPage === -1
                      ...
                      : `Affichage de ${Math.min(
                          ...
                          filtered.length,
                        )} sur ${filtered.length} catégories`}
                  </span>
                </div>
                ...
                {rowsPerPage !== -1 && totalPages > 1 && (
                  <div className="flex items-center gap-1">
                    ...
                    {windowPages.map((p, idx) =>
                      p === '…' ? (
                        <span key={`ellipsis-${idx}`} className="px-2 select-none text-slate-400">
                          …
                        </span>
                      ) : (
                        <Button
